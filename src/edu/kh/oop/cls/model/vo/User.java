...
public class User {
	...
	// 데이터 직접 접근 불가 원칙 -> 필드는 기본적으로 모두 private으로 작성(캡슐화)
	...
	// 생성자 : new 연산자를 통해서 객체가 생성될 때 생성된 객체의 필드값 초기화 + 기능 수행하는 역할
	...
	public User(String userId, String userPw, String userName, int userAge, char userGender) {
		...
		// 생성자 내에서 반드시 첫번째 줄에 작성되어야한다
		// 위에 다른 코드쓰면 에러
		// "Constructor call must be the first statement in a constructor"
		// 왜 사용하는가?
		// 중복 코드 제거, 코드 길이 감소, 재사용성 증가
		// 가독성이 좀 어려운 경우가 생길 수있어 많이 사용되지는 않음
	...
	// [오버로딩 조건]
	// 1) 메서드의 이름이 동일해야한다
	// 2) 매개변수의 개수, 타입, 순서 중 1개라도 달라야함. (변수명은 전혀 상관이 없음)
	
	//public User() {	} // 기본 생성자가 이미 작성되어있어서 중복
	public User(String userId) {} // 매개변수의 개수가 같은 생성자가 없어 오버로딩 성립
	public User(int userAge) {} // 매개변수의 개수가 같지만, 타입이 다름. 오버로딩 성립
	public User(String userId, int userAge) {} // 매개변수의 개수가 같지만, 한개의 타입이 다름. 오버로딩 성립
	public User(int userAge, String userId) {} // 매개변수의 개수, 타입이 모두 동일하지만 순서가 다름. 오버로딩 성립
	//public User(int userAge, String userName) {} // 매개변수의 갯수와, 타입, 순서가 모두 같아 오버로딩 불가. 변수명은 신경쓰지않는다.
